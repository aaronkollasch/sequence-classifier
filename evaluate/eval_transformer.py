...
parser = argparse.ArgumentParser(description="Evaluate a Transformer classifier model.")
...
parser.add_argument("--dropout-p-transformer", type=float, default=0.,
...
print("Loading data.")
dataset = data_loaders.IPIFastaDataset(
    ...
    kmer_params=checkpoint['dataset_params']['kmer_params'],
)
...
output, logits = trainer.test(model_eval=False, num_samples=args.num_samples, raw_output=True)
